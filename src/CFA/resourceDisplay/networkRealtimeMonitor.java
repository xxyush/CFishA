...
//连接数据库使用的
import java.sql.*;
public class networkRealtimeMonitor extends HttpServlet
{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -8948807238466553529L;
	/*绘图基本参数设置*/
	...
	//创建Servlet的构造方法、初始化方法和销毁方法
     ...
     public void setColor(double rate,Graphics2D g)//设置不同颜色
     {
    	 if(rate<=0.20)
    		 g.setColor(Color.green);
    	 else if(rate<=0.6&&rate>0.4)
    		 g.setColor(Color.blue);
    	 else if(rate<=0.8&&rate>0.6)
  ...
  */
     ...
     {
    	 networkRealtimeMonitor.this.boneNetR=30;
   	     networkRealtimeMonitor.this.boneNetLongR=80;
   	     networkRealtimeMonitor.this.componentwidth=25;
   	     networkRealtimeMonitor.this.componentheight=15;
 ...
 //绘制一个图片
     public void networkRealtimeMonitorImage(ServletOutputStream out)
   {
...
//处理客户端请求并调用createImage()方法向客户端输出了一个图片
     public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException{
        ...
        networkRealtimeMonitorImage(response.getOutputStream());
      }
}